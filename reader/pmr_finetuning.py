...
def _load_annotationsP_A(anno_folder, split):
    """Build an index out of FOIL annotations, mapping each image ID with its corresponding captions."""
    entries = []
    for file_name in __all__[split]:
        data_json = os.path.join(anno_folder,file_name)
        with open(data_json,"r+",encoding='utf8') as f:
            ...
            for item in data_json:
                ...
                if split == 'test':
                    ...
                    ans_label = item["answer_label"]
                img_id = _converId(item["img_id"])
                item_id = int(item["total_id"])
                entries.append(
                         {"question": premise,
                          "answers": item["answer_choices"],
...
class PMRDataReader:
    ...
    def __init__(self,
        ...
        anno_folder = self.task_conf['anno_folder']
        self.num_choice = int(self.task_conf['num_choice'])
        self._entries = _load_annotationsP_A(anno_folder,split)
        self._split = split
        self._names = []
    ...
    def data_generator(self):
        ...
        def wrapper():
            ...
            for epoch_index in range(self.epoch):
                ...
                if self.shuffle:
                    if epoch_index == 0:
                        self.global_rng.shuffle(list(sample_indice))
...
class PMRDataJointReader(object):
    ...
    def __init__(self,
        ...
        for task_index, task_conf in enumerate(task_conf_group):
            self.task_readers.append(PMRDataReader(task_conf, split, vocab_path, batch_size, shuffle,
