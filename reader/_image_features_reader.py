...
class ImageFeaturesH5Reader(object):
    ...
    def __getitem__(self, image_id):
        ...
        with self.env.begin(write=False) as txn:
            ...
            parse_boxes = self._parse_bytes(item['boxes'])
            parse_fea = self._parse_bytes(item["features"])


            features = np.frombuffer(base64.b64decode(parse_fea), dtype=np.float32).reshape(num_boxes, 2048)
            boxes = np.frombuffer(base64.b64decode(parse_boxes), dtype=np.float32).reshape(num_boxes, 4)
